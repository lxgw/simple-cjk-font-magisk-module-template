...
> - 本模板内容及相关文档**已过时**，本人已无力继续适配和维护，因此本模板可能存在[**兼容性问题**](https://github.com/lxgw/simple-cjk-font-magisk-module-template/issues)。
>
> **📜 免责声明**
...
>
> **📌 项目状态**
> - **本项目已归档，仅供存档参考，不再维护和更新。**
> - 如需更高级的自定义方案（支持西文、中文、日文、韩语分别搭配），请参阅 [高级版字体模块模板](https://github.com/lxgw/advanced-cjk-font-magisk-module-template)。
> 
> **💡 建议**
...
1. 在 [Release](https://github.com/lxgw/simple-cjk-font-magisk-module-template/releases/latest) 界面下载最新版本的模块模板压缩包（`FontTemplate-Magisk204.zip`，基于 Magisk 20.4 模板，更高版本 Magisk 通常也兼容）。**注意：不要直接点击 "Download Zip" 下载整个仓库，请从 Release 附件中下载。**
...
3. 要使加入的字体能够正常显示，**字体文件须遵循以下命名规则**：
   - 将喜爱的字体文件按字重（粗细）及语言（或优先级）依次重命名为 `fontwy.ttf` 或 `fontwy.otf`，复制到模块的 `/system/fonts` 目录下，刷入后重启即可。**重命名方式如下：**
     ...
     **例如：** `fontw4.ttf` 表示正文字重，`fontw7.otf` 表示粗体。
...
4. **最低 Magisk 版本要求**：本模板最低支持 **Magisk 20.4**。

### ROOT 隐藏兼容性（针对 Android 12+）

5. 如需在 Android 12 及以上系统隐藏 ROOT 的同时使用字体模块，建议：
